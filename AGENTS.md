...
- **取り込み状態の管理**: 共有・ダウンロード後のモーダルで「MoneyForward MEで保存した」ボタンを押すと、そのファイルが「確認済み」としてUIに記録される。これはMoneyForward ME側の状態を取得したものではなく、現在の画面内だけで保持されるユーザー確認状態である。ページを再読み込みすると失われる。
