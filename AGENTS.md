...
- **取り込み状態の管理**: 共有・ダウンロード後のモーダルで「MoneyForward MEで保存した」ボタンを押すと、そのファイル内の明細キーと件数を端末内へ保存し、次回以降の除外に使う。これはMoneyForward ME側の状態を取得したものではなく、ユーザーの確認に基づく記録である。新しいMoneyForward ME CSVを追加した際は、CSV内で確認できた分だけこの記録から差し引く。

- **MoneyForward ME CSVの追加と置換**: Share Targetから受け取ったMoneyForward ME CSVは、保存済み除外情報へ追加する。同じファイルは内容ハッシュで重複排除する。画面の「読み込み直す」から明示的に選択した場合だけ、保存済み除外情報を新しいCSVの内容で置き換える。「今回は除外せずに進む」はMoneyForward ME CSVも保存済み除外情報もない場合の意思確認として表示する。

- **変換用と監査用の選択状態**: 変換用と監査用のMoneyForward ME CSVは別々に保持する。Share Targetから受け取ったCSVは両方へ追加するが、監査画面での選び直しによって変換時の除外対象を置き換えない。
