...
- **削除候補リストの生成**: 誤って重複取り込みや別口座への取り込みをしてしまった場合に備え、PayPay明細とMoneyForward ME明細を突き合わせて削除候補CSVを生成します。
...
2. **MoneyForward ME明細を選択**: 調査したい期間を含むMoneyForward MEのCSVを選択します。
3. **要確認明細を確認**: 重複取り込みや異なる口座への取り込みが疑われる明細を画面上で確認します。
4. **確認用CSVをダウンロード**: MoneyForward ME上で削除すべき明細を探すための一覧として利用します。アプリが明細を自動削除することはありません。
