...
2. **既存明細を除外（推奨）**: MoneyForward MEからエクスポートしたCSVを追加すると、登録済みと判定された取引を出力対象から除外します。CSVを使わない場合は、明示的に「今回は除外せずに進む」を押します。一度読み込んだ登録済み明細の情報は端末内に保存されるため、次回からはPayPay CSVだけで変換できます。
