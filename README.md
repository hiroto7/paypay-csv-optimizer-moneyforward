...
MoneyForward MEにはPayPayの取引履歴を自動取得する機能がありません。その代わりに、PayPayから書き出した取引履歴のCSVファイルを読み込んで明細を登録できます。
...
- **重複登録・口座間違いの確認**: PayPayの取引履歴に同じ日付・金額・内容の取引があり、重複登録または異なる口座への登録の可能性があるMoneyForward MEの明細を表示します。重複か口座間違いかは自動で判定しません。
...
併用払いを支払い方法ごとに分けた後、「PayPay」で始まる支払い方法の明細だけを取り込み用CSVと重複登録・口座間違いの確認対象にします。クレジットカードや銀行口座などを使用した明細は対象外です。

## 使い方

1. PayPayから取引履歴をダウンロードし、そのCSVファイルをPP2MFで選びます。
2. MoneyForward MEから書き出した入出金履歴を追加します（任意）。追加すると、登録済みの明細を作成対象から除外します。
